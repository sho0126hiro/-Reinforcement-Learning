...
# reference: https://gist.github.com/mizchi/71b965baf19b3fa7355a9df2b920a82c
# reference: https://mizchi.hatenablog.com/entry/2018/10/28/024934
# reference: 
import random
import copy
...
class Solver:
    ...
    def replay_experience(self, batch_size: int, dump_and_stop=False):
        """
        target_f: 教師信号（回答）
        input_action: 入力 [状態,行動]: list
        x -> 入力, y -> 出力
        model.fit: モデルの訓練
        model.fit(入力，出力，．．．)
        """
